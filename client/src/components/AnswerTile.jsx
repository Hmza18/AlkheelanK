...
export default function AnswerTile({
  ...
  return (
    ...
    >
      {isTF ? (
        <span className="text-4xl sm:text-5xl">{s.glyph}</span>
      ) : (
        <span className={`flex shrink-0 items-center justify-center ${compact ? "h-7 w-7" : "h-10 w-10"}`}>
          <Shape type={s.shape} size={compact ? 24 : big ? 38 : 30} />
        </span>
      )}
      <span className={`leading-tight drop-shadow ${isTF ? "text-3xl sm:text-4xl" : "flex-1 text-left"}`}>
        {text}
      </span>
